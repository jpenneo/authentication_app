...
function isAuthenticated() {
  ...
  // Aquí podrías agregar lógica para verificar si el token es válido, por ejemplo:
  try {
    const decodedToken = JSON.parse(atob(token.split(".")[1])); // Decodificar el JWT (sin verificar aún)
    const currentTime = Math.floor(Date.now() / 1000); // Tiempo actual en segundos

    // Verifica si el token no ha expirado
    return decodedToken.exp > currentTime;
  } catch (e) {
    return false; // Si no se puede decodificar el token, no está autenticado
  }
}
...
router.beforeEach((to, from, next) => {
  ...
  const isAuthenticatedUser = isAuthenticated(); // Usar la función para verificar si el usuario tiene un token válido

  if (requiresAuth && !isAuthenticatedUser) {
    next("/inicio-sesion"); // Redirige a login si no está autenticado
  } else {
    next(); // Permite la navegación si el usuario está autenticado
  }
});
